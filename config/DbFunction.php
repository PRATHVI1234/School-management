...
class DbFunction{
	...
	function login($loginid,$password,$type){
      ...
      if(!ctype_alpha($loginid) || !ctype_alpha($password)){
        ...
        echo "<script>alert('Either LoginId or Password is Missing')</script>";
			...
			$query = "SELECT loginid,password,type FROM tbl_login where loginid=? and password=? and type=?";
			...
			if(false===$stmt){
				...
				$stmt->bind_param('sss',$loginid,$password,$type);
				$stmt->execute();
				$stmt -> bind_result($loginid,$password,$type);
				$rs=$stmt->fetch();
				if(!$rs)
				{
				...
				else
				{
						...
						{
							header('location:teachers\dashboard1.php');
						}
						else {
							header('location:Parent/view-course.php');
						}
	...
	function feedback($name,$date,$feedback){
		if($feedback==""){

			echo "<script>alert('Please Input Feedback That You Want To POST')</script>";
			...
			$db = Database::getInstance();
			$mysqli = $db->getConnection();
			$query = "insert into feedback(name,date,feedback) values(?,?,?)";
			$stmt= $mysqli->prepare($query);
			if(false===$stmt){
				...
				$stmt->bind_param('sss',$name,$date,$feedback);
				$stmt->execute();
	...
	function reglogin($loginid,$password,$type){
		...
		if(!ctype_alpha($loginid) || !ctype_alpha($password)){
		  ...
		  echo "<script>alert('Either LoginId or Password is Missing')</script>";
			  ...
			  if(false===$stmt){
				  ...
				  if(!$rs)
				  {
					echo"<script>alert('You dont have permission to Register')</script>";
				}
				  else
				  {
					header('location:../Registration/Addcoordinator.php');
	  ...
	  function create_coordinator($username,$type,$password)
	  {
		if($username==""){
			...
			$db = Database::getInstance();
			$mysqli = $db->getConnection();
			$query = "insert into tbl_login(loginid,type,password)values(?,?,?)";
			$stmt= $mysqli->prepare($query);
			if(false===$stmt){
				...
				$stmt->bind_param('sss',$username,$type,$password);
				$stmt->execute();
...
function create_subject($cshort,$cfull,$sub1,$sub2,$sub3){
	...
	function faculty($cshort,$cfull,$sub1,$sub2,$sub3){
				...
				$db = Database::getInstance();
				$mysqli = $db->getConnection();
				$query = "insert into faculty(fname,fmob,sub1,sub2,sub3)values(?,?,?,?,?)";
				$stmt= $mysqli->prepare($query);
				if(false===$stmt){
					...
					$stmt->bind_param('sisss',$cshort,$cfull,$sub1,$sub2,$sub3);
					$stmt->execute();
					echo "<script>alert('Course Added Successfully')</script>";
...
function del_faculty($id){

		//echo $id;exit;
	 $db = Database::getInstance();
	 $mysqli = $db->getConnection();
	 $query="delete from faculty where subid=?";
	 $stmt= $mysqli->prepare($query);
	 $stmt->bind_param('i',$id);
 $stmt->execute();
...
function notices($cshort,$cdate){

	if($cshort==""){

echo "<script>alert('Please Input Notice That You Want To POST')</script>";
...
$db = Database::getInstance();
$mysqli = $db->getConnection();
$query = "insert into notices(Notice,date) values(?,?)";
$stmt= $mysqli->prepare($query);
if(false===$stmt){
